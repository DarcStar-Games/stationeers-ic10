...
A deployable framework of **185 Stationeers IC10 assembly programs** (`ic10/**/*.ic10`) plus a Python
...
```bash
python3 tools/run_validation.py                   # full suite: 36 validators + 52 protocol/execution tests
...
| Path | Holds | Notes |
|---|---|---|
| `ic10/<deployment-family>/` | the product: 185 production IC10 programs | never generated by hand except where noted below |
...
`validation/validators/validate_logic_enum_tokens.py` does the same for operands: every bare or dotted
LogicType and LogicSlotType name must be a member of the enum in `data/logic_enums.json`, extracted from
the target build's assembly, in a position the game signature gives that enum (issue #326).
...
47 of 185 programs sit at ≥117 lines and 17 hold a reviewed `SOFT_LIMIT_EXEMPTIONS` entry in
