...
def generate_recipe_fixture(output):
    ...
    output=Path(output)
    subprocess.run([sys.executable,str(R/'generate_recipe_catalog.py'),'--game-data',str(R/'recipe_fixture_data'),'--output',str(output),'--clean'],check=True,stdout=subprocess.DEVNULL)
