...
### Wiring and configuration
Follow `docs/STOCK_TARGET_INGRESS.md` for continuous targets and `docs/OPERATOR_ORDER_INGRESS.md` for one-shot orders. Stock Job Ingress exclusively owns Gateway lane E. Every dependency service the stock-target views post to is an instance of its own; only the Job Store, Plan Store, Reservation Snapshot Host, Config Host, and the two profile views are shared, and those are read or re-validated, never posted to. Operator Order Ingress exclusively owns lane F and wires its Recipe View to dedicated Lookup and exact Execution Profile service instances; do not share those request mailboxes with the resident print pipeline. Executor `d1` points to Dependency Plan Store.
