...
3. Export the programs needed for the deployment with `python3 tools/export_to_game.py --output <Stationeers-scripts-dir> --family <family>`. Use `--program <unique-stem>` for individual programs or `--all` for the complete framework. The game displays a compact human title without the family or source revision; XML `Description` records the deployment family, revision, and canonical source path. The collision-safe save directory remains `family__program`; existing exports require an explicit `--overwrite`, and directories containing unrelated files are never reused.
...
`docs/DIRECTORY_STANDARD.md`, `docs/ARCHITECTURE.md`, `docs/CONTROLLER_DIRECTORY_GETTING_STARTED.md`
...
### Technical references
`docs/DIRECTORY_STANDARD.md`, `docs/COMMISSIONING_QUICKSTART.md`, `docs/CONTROLLER_DIRECTORY_GETTING_STARTED.md`
...
| `ic10/manufacturing-ingress/stock_target_inventory_view_v1_0.ic10` | `conditional-resident` | Quotes coherent ITEM stock and distinguishes exact deficits from lower-bound ambiguity and overflow. |
| `ic10/manufacturing-ingress/stock_target_job_evaluator_v1_0.ic10` | `conditional-resident` | Round-robins four configured stock targets and submits one root intent only for a proven deficit. |
| `ic10/manufacturing-ingress/stock_target_job_ingress_v1_0.ic10` | `conditional-resident` | Revalidates producer metadata, demand, and config generation before restart-safe Gateway lane-E root publication. |
| `ic10/manufacturing-ingress/stock_target_producer_view_v1_0.ic10` | `conditional-resident` | Serializes evaluation and mutation-time resolution of target ITEMs to canonical root intent and output-per-batch metadata. |
<!-- FAMILY_PROGRAMS:manufacturing-ingress END -->

### Prerequisites
Job Store/Gateway ABI5/Executor, Dependency Plan Store, and healthy Transform/Print execution. Stock targets also need Config Host and their own instances of the ITEM Reservation Selector, Item Producer Resolver, Job Requirement View with its Reagent Resolver, and Claim View with its Child Validity and Job Monitor; those seven are never shared with a resident dependency planner, which runs on its own loop (`docs/STOCK_TARGET_INGRESS.md`). Operator orders need Generic Input Scanner/Resolver, a dedicated Recipe Lookup, a dedicated Recipe Execution Profile View, and a commit Switch.

### Wiring and configuration
Follow `docs/STOCK_TARGET_INGRESS.md` for continuous targets and `docs/OPERATOR_ORDER_INGRESS.md` for one-shot orders. Stock Job Ingress exclusively owns Gateway lane E. Every dependency service the stock-target views post to is an instance of its own; what the family shares with a resident planner is read only (Job Store, Plan Store, Reservation Snapshot Host), re-validated after selection (the two profile views), or laned (Gateway lane E). Operator Order Ingress exclusively owns lane F and wires its Recipe View to dedicated Lookup and exact Execution Profile service instances; do not share those request mailboxes with the resident print pipeline. Executor `d1` points to Dependency Plan Store.
...
### Wiring and configuration
The Job Inventory Preflight checks inventory; Item Producer Resolver, Job Requirement View, and Manufacturing Reagent Resolver normalize producer/requirements; Dependency Plan Store persists plans; Manufacturing Dependency Planner is the sole plan-mutation coordinator; the child creator/build/new-plan controllers create new plans/children; plan evaluator/child-validity/existing-plan controller evaluate existing plans; the ancestry guard bounds depth/cycles; claim/release services handle shared claims; Manufacturing Dependency Gate gates parent execution; cleanup flows through Dependency Cancellation Guard and Job Gateway. The Cancellation Guard's `d1` is a Job Monitor of its own; the Planner's tree (Child Validity, Ancestry Guard, New controller) shares another, because the Guard scans on its own loop and a Monitor mailbox holds one request. A stock-target deployment likewise brings its own Selector, Resolver, Requirement View, Reagent Resolver, Claim View, Child Validity, and Job Monitor rather than posting into these (`data/mailbox_arbitration.json`).
...
## Live Commissioning and Inspection Tools
...
### Common failures
Monitor NaN caused by a missing target/selector or invalid address, fingerprint mismatch, missing required case, ambiguous physical action, or a probe used as authority. Check the monitor status contract; for evidence failures mark BLOCKED/FAIL and never edit evidence to manufacture PASS.

### Reflash / replacement
Reflash either tool freely; neither owns actuation state. A new framework release requires a new/current session fingerprint.

### What can be removed
Always on-demand. Reclaim after the field session.

### Technical references
`docs/LIVE_COMMISSIONING.md`, `docs/FRAMEWORK_HARDENING_TESTS.md`, `docs/STACK_CELL_MONITOR_GETTING_STARTED.md`
