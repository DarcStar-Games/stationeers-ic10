...
def render(s):
 ...
 lines=[f"# {s['comment']}",'Boot:','clr db','poke 0 31415983','poke 1 2',f'poke 2 HASH("{s["schema"]}")','poke 3 1',f'poke 4 {w}','poke 5 64','poke 10 1','Loop:','yield','get r0 db 11','beqz r0 ScanStart','poke 12 r0','j Loop','ScanStart:','poke 12 0','get r15 db 8','add r15 r15 1','poke 8 r15','poke 9 0','move r7 0','move r8 0','Scan:','get r1 db:0 r7','blt r1 0 Publish','add r7 r7 1','ld r0 r1 PrefabHash','beq r0 -128473777 Probe','bne r0 2037291645 Scan','Probe:','getd r0 r1 0',f'bne r0 {s["provider_magic"]} Scan','getd r0 r1 1',f'bne r0 {s["provider_abi"]} Scan',f'getd r15 r1 {g}','blez r15 Scan']
