...
Item 8 preserves the Scheduler's queue/lifecycle policy while removing direct multiwriter access to the Generic Job Store command mailbox. `ic10/generic-jobs/generic_job_command_gateway_v3_0.ic10` (Job Command Gateway ABI3) has four independent producer lanes: Scheduler lifecycle, Planner cancellation, Child creation, and POWER lifecycle. `ic10/generic-jobs/generic_job_store_command_executor_v1_0.ic10` is the **sole physical Job Store mailbox writer** for those production paths.
...
`ic10/manufacturing/manufacturing_scheduler_v1_0.ic10` remains the manufacturing lifecycle-policy owner: it decides legal Job ABI edges and wait/fault outcomes. It publishes those decisions through Gateway lane A rather than writing `ic10/generic-jobs/generic_job_store_v1_0.ic10` directly. Planner cancellation uses lane B; Child Creator uses lane C. `ic10/generic-jobs/generic_job_store_command_executor_v1_0.ic10` serializes all four into the one Job Store request mailbox; manufacturing/dependency use lanes A/B/C and power uses lane D.
