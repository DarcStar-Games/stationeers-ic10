...
8. Field locations are service-specific. The protocol standardizes semantics and ordering, not absolute stack addresses. Reviewed against the field map on `2def91f` (2026-09-13, issue #191): the 86 request-token entries sit at 29 distinct starting cells and the most common, `S18`, holds 10 of them; the 80 response-token entries sit at 27, with `S10` holding 14; the 106 state entries sit at 28, with `S11` holding 13. A shape that ratified each plurality would move 76 request-token and 66 response-token entries, 16 of whose providers sit at the 120-line ceiling, and would change the `S0` identity every consumer of each moved provider compares. Of the 79 programs with a contract consumer edge, 54 consume one layout and 25 consume two or more, and only 2 of those 25 already see one token cell across all their providers. A shared offset would not let a fixed-pin consumer share its fencing code, because `get r0 d1 10` and `get r0 d2 10` are separate instructions whatever the offsets; it pays only through a register-indexed pin or across lanes inside one provider, and both places the tree has one already share the offsets that pay: the Power Job Scheduler posts one request image to Prepare and Finalize through `dr9`, whose layouts are identical, and the Command Gateway keeps both tokens of lanes B to F at the first two cells of each 16-cell lane so one loop scans all five. Invariant 8 therefore stands, with that carve-in: mailboxes one caller reaches through one register-indexed pin, or lanes one provider scans with one loop, share the offsets that loop reads; nothing else is renumbered for alignment's sake. `docs/STACK_FIELD_MAP.md` lists every role's cells with peers, consumers, provider headroom, and the most common starting cells, so the review can be repeated on a later tree.
