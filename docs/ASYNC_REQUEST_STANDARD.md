...
8. Field locations are service-specific. The protocol standardizes semantics and ordering, not absolute stack addresses. Reviewed against the field map on `2def91f` (2026-09-13, issue #191): the 86 request-token entries sit at 29 distinct starting cells and the most common, `S18`, holds 10 of them; the 80 response-token entries sit at 27, with `S10` holding 14; the 106 state entries sit at 28, with `S11` holding 13. A shape that ratified each plurality would move 76 request-token and 66 response-token entries, 16 of whose providers sit at or over the 120-line ceiling, and would change the `S0` identity every consumer of each moved provider compares. Of the 79 programs with a contract consumer edge, 54 consume one layout and 25 consume two or more, and only 2 of those 25 already see one token cell across all their providers. A shared offset would not let a fixed-pin consumer share its fencing code, because `get r0 d1 10` and `get r0 d2 10` are separate instructions whatever the offsets; it pays only through a register-indexed pin or across lanes inside one provider, and the three places the tree has one already share the offsets that pay: the Power Job Scheduler posts one request image to Prepare and Finalize through `dr9`, whose layouts are identical; the Command Gateway keeps both tokens of lanes B to F at the first two cells of each 16-cell lane so one loop scans all five; and the Printer Execution Bank serves six per-pin lanes, request at S32, request token at S40, response token at S56, each plus the pin, with one loop body its Capacity Client addresses the same way. Invariant 8 therefore stands, with that carve-in: mailboxes one caller reaches through one register-indexed pin, or lanes one provider scans with one loop, share the offsets that loop reads; nothing else is renumbered for alignment's sake. `docs/STACK_FIELD_MAP.md` lists every role's cells with peers, consumers, provider headroom, and the most common starting cells, so the review can be repeated on a later tree.
9. A mailbox instance has one writer at a time. Token identity fences what a caller reads; it does not order who posts, and a second caller can replace an unlatched request and strand the first. Independent callers use separate lanes (disjoint request cells the callee serves one at a time) or separate instances. `data/mailbox_arbitration.json` records the arbitration of every mailbox more than one program writes, and `validation/validators/validate_mailbox_arbitration.py` checks it against the wiring map (`docs/SCRIPT_WIRING.md`). For a `serial` group that validator also proves the waiting: every program from the root to a writer reads each peer's response token, or a `LIVE_CURRENT` producer's current token, before it writes another peer's request token or its own response token (`framework/request_blocking.py`, issue #146).
10. Consecutive postings into one mailbox carry different tokens unless the second retries the first with the same payload (invariant 6). The fence in invariant 4 tells a new reply from the previous one by nothing but the token: a callee handed the token its response cell already holds never latches the request, and the caller's wait passes on its first poll with the earlier reply. A posting whose payload is re-read from state that changed, such as a scan restarted by a coherence check, is a new logical request even when it reaches the same position, and carries a new token. A caller that posts once per position under one request therefore derives each token from a per-request posting counter rather than the position, `request * 512 + posting`, with the counter reset when the request is answered and the 512th posting failing the request, so one request's tokens never reach the next request's. `framework/async_request.py` holds the derivation as `posting_token`; `ic10/dependency-planning/dependency_claim_view_v1_0.ic10`, `ic10/manufacturing-ingress/stock_target_future_view_v1_0.ic10`, and `ic10/dependency-planning/dependency_ancestry_guard_v1_0.ic10` are the scanning callers that restart (issue #148). The Planner and the Preflight derive tokens from position but never post again after a wait. The count lives in a private cell of the caller's own stack beside the request, written before each posting is published and cleared after the reply token, so a caller reflashed while a request is in flight continues the count it had rather than repeating a token (issue #182). The idle tick clears the cell, so a housing that held other state is clean before it serves a request, unless one is already in flight at boot, when the cell is trusted as the count the way the request cells are trusted as the request. A reflash between the reply token and the clear leaves a gap in the next request's tokens, never a repeat.
