...
The Allocator validates a stable Link Resolver publication, holds its input count to the sixteen four-cell records a Resolver's table can hold (`S20..S83`) and rejects a larger one before it stages anything (issue #151), allocates a new candidate epoch, and commands the Stager to prepare all one-to-three inputs.
...
The activation sequence is snapshot, `Activate`, then the state cell last. A reflash or power loss between `Activate` and the state publish restarts the Runtime in `WaitInput` with the furnace already running; it takes a second snapshot and activates again, and the job then needs the declared growth past that later snapshot. `tests/test_fault_injection.py` cuts the run after every instruction from the request to completion and bounds the cost: the over-run is at most what the furnace produced while the Runtime was down plus its boot tick, and a cut inside the activation window costs exactly that. The other order, state before `Activate`, leaves a job in `WaitOutput` with a furnace that never started, waiting out the 512-tick fault; the same walk shows that order stranding both window cuts for 512 ticks (issue #165).

The same walk found two fail-open restarts in the Runtime's idle path and removed them. A served token no longer rewrites the status: `S20` keeps what the request last published, `1` complete or `-1` fault, until the next request, where before an idle tick wrote `1` over a fault one tick after publishing it. A restart with the state cell at `0` resumes what the status says was in flight: status `2` with no state re-runs the acceptance, which re-validates and re-posts the same token to the Allocator, and status `3` or `4` with no state reasserts the fault whose status never landed. Before that, a reflash anywhere between binding the token and posting the Allocator request reported the never-posted job complete under its token. Completion publishes status `1` before it clears the state cell, so a reflash between the two re-runs the output check once rather than reading as an interrupted fault; a caller that takes the `1` and posts its next request inside that one tick can change the batch count under the re-check, which then waits out the 512-tick fault before the next request is taken up. The fault notice the Runtime posts to the Allocator carries the token it served, `S21`, not the caller's request cell, so that late fault cannot cancel the request that follows it.

A Runtime outage that spans the Allocator's committed window costs the job nothing but the wait (issue #204). Allocator ABI2 moves its status `S22` from `1` committed through `4` cleaning to `2` delivered on its own clock, zeroes `S14` as it leaves `1`, and echoes the request token at `S16` only after it has published the commit, in the same tick. The Runtime therefore takes the echo as the commit and never requires `S22` to read `1`; the epoch it mirrors comes from `S15`, the completed epoch, once `S22` reads `2`. Before that, `WaitAlloc` accepted `S22` only at exactly `1` and read the epoch from `S14`, so a Runtime that was down while the Allocator delivered, a power loss on its housing alone with the Feeders still running, came back to a `2` it would not take and an `S14` of zero it would have faulted on, and waited forever with status `2` under its token while the inputs were consumed and no transform ran. The walk in `tests/test_fault_injection.py` delivers the inputs inside the outage, so every cut from the request post onward resumes to an Allocator that has already moved on; its control, the old wait shape, strands exactly the cuts from the post to the state publish.
