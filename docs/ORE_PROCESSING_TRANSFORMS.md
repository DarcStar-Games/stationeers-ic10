...
The staged count at S8 is published after each record rather than once at the end, so a Stager interrupted mid-staging restarts holding the number of records it actually wrote, and a later clean undoes exactly those. That only holds while the stack is its own. A reflash preserves every cell, so a housing that last ran something else hands the Stager both a staged count at S8 and an unread request across S9..S12 and S14 — an inherited `S12 != S14` with an inherited `S9` naming a command is indistinguishable, cell for cell, from a request the live Allocator just wrote. Boot therefore admits **none** of it unless the stack already carries the `MultiMaterialReservationStager.v1` identity the Stager publishes at S0, and clears the stack outright on any identity that does not match. A housing that last ran something else cleans nothing and stages nothing; one that ran this same program resumes with its count and its unfinished request intact. Because S0 folds the ABI into its hash, an earlier revision of the Stager is disowned on the same test.
