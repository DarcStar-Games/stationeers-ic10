...
| Transform Runtime | reflash after every instruction from the request to completion, the furnace running on through the outage | every cut completes only on the declared growth past a snapshot taken after the epoch committed, never waits on a furnace it did not switch on, and over-runs by at most the outage production; a published fault stays published |
