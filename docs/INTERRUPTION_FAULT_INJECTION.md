...
| Transform Runtime | reflash after every instruction from the request to completion, the furnace running on and the Allocator delivering through the outage | every cut completes only on the declared growth past a snapshot taken after the epoch committed, never waits on a furnace it did not switch on or an Allocator status the outage consumed, and over-runs by at most the outage production; a published fault stays published |
...
The Transform Runtime walk (issue #165) cuts the real IC10 after every instruction from the request to completion, with the furnace running on through a three-tick outage. It bounded the cost of the activation order the program already had, snapshot then `Activate` then the state cell: a cut between `Activate` and the state publish costs exactly the furnace's output over the outage and the boot tick, and no cut costs more. It also found two restarts that reported a job complete with nothing done: a reflash between binding the request token and posting the Allocator request, whose restart took the served token as an idle housing and published status `1`; and the idle tick itself, which wrote `1` over a published fault one tick later. `ic10/material-transform/generic_material_transform_runtime_v2_0.ic10` now leaves the published status alone under a served token and resumes an interrupted acceptance or reasserts an interrupted fault from the status cell; the control in the test, state published before `Activate`, strands both window cuts for the 512-tick timeout. With the Allocator's delivery moved inside the outage (issue #204), the same walk stranded the original program at fifteen cuts, from the request post to its epoch mirror: `WaitAlloc` accepted the Allocator's `S22` only at exactly `1`, a value the outage had consumed, and the epoch it then read from `S14` was already zero. The Runtime now takes the Allocator's echo of its token as the commit and mirrors the epoch from `S15` once delivery reads `2`; the second control in the test, a wait that must witness the `1`, strands exactly the twelve cuts from the post to the state publish, with status `2` under the served token.
