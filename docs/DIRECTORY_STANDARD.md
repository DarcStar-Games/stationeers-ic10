...
Of the request block, the Bridge writes exactly `S9`, `S11`, `S12`, `S14`, `S16..S19`, and `S22`, and that is what the Host's contract declares externally writable. `S20` and `S21` are the Host's own rebuild state — the bank select is written only as `seq r6 r5 0` from the active bank at `S24`, and the staging count only as the Host increments it — so they are not in the declaration, and the Host's reviewed stack windows rest on that: a peer that wrote `S20` would break the premise the bank offset is derived from.
