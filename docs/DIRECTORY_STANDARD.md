...
Of the request block, the Bridge writes exactly `S9`, `S11`, `S12`, `S14`, `S16..S19`, and `S22`, and that is what the Host's contract declares externally writable. `S20` and `S21` are the Host's own rebuild state — the bank select is written only as `seq r6 r5 0` from the active bank at `S24`, and the staging count only by the Host, cleared at BEGIN and incremented at ADD — so they are not in the declaration, and the Host's reviewed stack windows rest on that: a peer that wrote `S20` would break the premise the bank offset is derived from.
...
`ic10/directory-core/generic_directory_adapter_bridge_v1_0.ic10` validates Adapter ABI3 and snapshot mode, acquires the Adapter freeze handshake, captures generation/sequence, copies SchemaId/Version and geometry into the Host, propagates adapter overflow into the staging snapshot, sends complete records, revalidates the frozen generation/sequence before publication, compares the staged result with the active snapshot, and suppresses COMMIT when nothing changed. Before it writes anything it holds the Adapter's header to what the Host supports: a width above three, a capacity above 64, or a candidate count above the capacity releases the freeze and publishes nothing, so a malformed header cannot carry the candidate copy across the Host's request block into its rebuild state, or walk the record copy past the Adapter's table (issue #151). Each bound is a ceiling only: a count below zero drives the same empty copy a count of zero does, and a width or capacity below one copies nothing and is refused by the Host, so no floor is held.
...
The discovery adapter supplies six cells `[NodeId, ReferenceId, State, UsedCells, AssignmentEpoch, CatalogInstanceId]`, deduplicating NodeId before publication and faulting duplicate live Stores. The Registry Host freezes and validates exactly `DirectorySchema.CatalogStoreNode` v1, indexes by NodeId, rejects overflow/incoherence, marks absent previously known nodes MISSING, preserves RETIRED records, and publishes the schema/version with S23 written even LAST. A candidate count above the 64-node capacity is rejected as a malformed source (`S16 = -4`) before any record is read, which holds the candidate scan to `S18..S401`; the reviewed window the Host's contract had carried stopped at `S399`, two cells short of the last record's `AssignmentEpoch` and `CatalogInstanceId` (issue #151). The bound is a ceiling only; a count below zero drives the same empty scan a count of zero does. Consumers require S23 even and unchanged around every registry read that can trigger side effects.
