...
A search resumes across requests from register state, so the Enumerator resumes only on a stack its own image published: a boot that finds any other `S0` clears the housing before publishing its header, and the next request starts a fresh search (issue #142; `docs/PRESSURE_MULTI_HOP_ROUTING.md`). A fault (`S9 = -1`) also drops the resume key, so a repeated SearchId searches the current snapshot rather than re-checking the one that faulted (issue #169).
