...
Material ITEM Endpoints preserve `S14 ResourceProfileViewRef`, published once the Resource Profile View on `d1` has passed the endpoint's identity check and `0` from boot until then. Storage metadata uses S35..S40: AccessKind, PolicyFlags, ReserveFloor, FirstSourceSlot, FirstSourceQuantity, and FirstEmptySlot. Policy flags support DO_NOT_CONSUME, NO_IMPORT, PREFERRED_DESTINATION, and QUARANTINE.
