...
Lifecycle-edge legality is a writer contract in `generic_job_schema.json` / `framework/job_abi.py`: normal progress follows QUEUED -> PLANNING -> RESERVING -> READY -> RUNNING -> VERIFYING -> COMPLETE; planning/reservation/readiness may enter explicit wait states; FAULT/CANCELLED are terminal. Keeping that policy outside the 120-line Store prevents processor-specific scheduling behavior from leaking into the generic storage ABI.
