...
ck(s.get('storage_compatibility')=={'magic':-955081679,'abi':1,'rule':'both must match before durable stack geometry is interpreted'},'Job storage compatibility rule mismatch')
...
for token in ('bne r0 HASH("GenericJobStore.v1") Reset','get r0 db 1','beq r0 1 Recover','poke 0 HASH("GenericJobStore.v1")','poke 1 1','poke 18 32','poke 23 1','poke 24 r15','poke 25 r3','poke 26 r4','beq r10 3 Reap','beq r6 7 Respond','bgt r6 10 Respond','beq r6 7 ReapOK','blt r6 11 Respond','bgt r6 12 Respond'):
