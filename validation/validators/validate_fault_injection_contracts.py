...
ordered('ic10/generic-jobs/generic_job_command_gateway_v3_0.ic10','mul r0 r15 5','add r0 r0 r6','put d0 2 r0')
# Power Plan Store boot must invalidate an interrupted odd COMMIT before restoring even sequence.
ordered('ic10/power-grid/power_dispatch_plan_store_v1_0.ic10','and r1 r0 1','poke 3 0','poke 4 0','add r0 r0 1','poke 2 r0')
# New POWER reservations are committed before old release; allocator authority is published last.
ordered('ic10/power-grid/power_reservation_allocator_v1_0.ic10','poke 2 0','WaitC:','WaitR:','Publish:','poke 4 1')
# Executors are gated by allocator active flag and exact PlanGeneration.
ordered_after('ic10/power-grid/power_load_executor_v1_0.ic10','Set:','get r0 d1 4','get r0 d1 2','get r0 d1 3','Write:','sd r3 On r4')
ordered_after('ic10/power-grid/power_link_executor_v1_0.ic10','Set:','get r0 d1 4','get r0 d1 2','get r0 d1 3','Write:','sd r3 Setting r4','sd r3 On r5')
