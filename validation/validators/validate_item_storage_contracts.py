...
def txt(f): return (R/f).read_text()
def need(f,*xs):
 s=txt(f)
 for x in xs:
  if x not in s: fails.append(f'{f}: missing {x!r}')
def before(f,a,b):
 s=txt(f); ia=s.find(a); ib=s.find(b)
...
if 'yield' in body: fails.append('196 direct storage scan yields inside bounded snapshot')
...
print(' - ITEM storage uses one Generic Endpoint/Reservation model across Vending, LArRE, direct slots, chute export and SDB')
print(' - split reservation quotes are read-only; commits bind allocator identity/epoch, semantic mirror generation, and exact physical action hints')
print(' - physical LArRE movement requires current source and destination reservations before pickup and supports held-item recovery')
print(' - SDB inventory is conservative lower-bound while Stacker-backed dispense meters exact requested quantities')
