...
ck([freq.stack.get(i) for i in range(4,8)]==[23500,24000,600,100000],'furnace condition bounds not copied from Transform Profile')
ck(freq.stack.get(3)==H('Fuel.H2O2') and freq.stack.get(9)==H('AlloyInconel'),'furnace utility identity mismatch')
...
mixd=Device(315,stack={0:31416048,1:1,3:H('Fuel.H2O2'),4:100,10:1,11:1,12:1},props={'ReferenceId':315})
...
ck(gc.stack.get(3)==H('Fuel.H2O2') and gc.stack.get(4)==.1 and gc.stack.get(5)==1,'GFG fuel PressureDomain request wrong')
...
ck(to_pc(gv,56),'could not reach GFG final PowerPlan cut');pl.stack.update({2:4,5:0,6:0});gv.run(1)
