...
for n in ('poke 0 HASH("DirectoryAdapter.v3")','poke 3 HASH("DirectorySchema.Controller.v1")','poke 10 2','poke 11 64','poke 15 1'):
...
for n in ('poke 0 HASH("GenericSnapshotDirectoryHost.v1")','poke 1 1','bgt r3 64 Error','add r10 r6 29','poke r10 r9','mul r4 r2 r3'):
 if n not in H:fails.append('Generic Directory Host missing '+n)
for n in ('poke 1 2','HASH("DirectorySchema.Controller.v1")','bgtz r0 Overflow','Scan:','SameType:','getd r0 directory 24','bne r0 r10 Updating'):
 if n not in S:fails.append('Direct Controller Selector missing '+n)
if not all(x in A for x in ('bne r0 HASH("GenericSnapshotDirectoryHost.v1") BadDirectory','HASH("DirectorySchema.Controller.v1")','bgtz r0 BadDirectory')):fails.append('Arbiter does not validate generic schema/refuse overflow')
if not all(x in L for x in ('bne r0 HASH("GenericSnapshotDirectoryHost.v1") Publish','HASH("DirectorySchema.Controller.v1")','bgtz r0 SourceOverflow')):fails.append('Pressure Link adapter does not validate/refuse source snapshot')
...
dir.stack[29]=1;dir.stack[25]=10;vm.run(2,max_steps=10000)
