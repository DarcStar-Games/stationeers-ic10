...
for s in ('poke 0 HASH("ResourceEndpoint.v1")','poke 52 2','poke 54 2','get r14 d1 29','bne r0 HASH("ResourceProfileView.v1") Bad','get r0 d1 8','bne r0 2 Bad','ls r4 d0 0 Occupied','poke 11 r0'):need(e,s,'material import endpoint')
...
for tok in ('bne r0 HASH("GenericSnapshotDirectoryHost.v1") Bad','HASH("DirectorySchema.ResourceLink.v1")','getd r0 r1 22','poke r0 r1'):need(r,tok,'transform link resolver')
for tok in ('putd r2 14 r10','putd r3 15 r10','putd r4 17 r1','poke 13 1'):need(s,tok,'reservation stager')
for tok in ('poke 0 HASH("MultiMaterialReservationAllocator.v2")','poke 1 2','poke 14 r1','poke 14 0','put d1 9 2'):need(m,tok,'multi allocator')
...
for tok in ('clr db','poke 0 HASH("CatalogLoader.v5")','poke 1 5','poke 18 1 # immutable candidate publication LAST'):need(tl,tok,'transform catalog loader')
